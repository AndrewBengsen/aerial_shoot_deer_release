...
# Calculate total cost per hour with different helicopters and kill rates

total_cost_hr <- function(heli_hr, shtr = shtr_hr, nav = nav_hr, 
                          ammo_cost = ammo, deer = deer_hr, shots = shots_deer){
  # heli_hr = jet_hr or sql_hr
  # deer_hr = deer shot per hour
  hr_cost <- heli_hr + shtr + nav + (ammo_cost * deer * shots) #total cost per hr
  p_var <- (hr_cost - (heli_hr + shtr + nav)) / hr_cost * 100 # % variable costs
...
cat("model {
 ...
 for(i in 1:ncase){
  ...
  n[i] ~ dnorm(nhat[i], tau[i])
   tau[i] <- 1/(se_nhat[i]*se_nhat[i])
 ...
 for(i in 1:ncase){
     y[i] ~ dbin(p[i], round(nhat[i])) # need round() to make n an integer for dbin
     logit(p[i]) <- alpha[i]
  } #i
...
cat("model {
 ...
 shots_deer ~ dnorm(4.1, tau_shots)
  tau_shots <- 1/(2.2 * 2.2)
...
cat("model {
 ...
 # Need to find the right distribution for actual prediction
 for(k in 1:nkd){
    #pred_hrs[k] ~ dgamma(shape, pred_rate[k]) #dexp(pred_mean[k])  
    # convert the mean to the rate parameter
    #pred_rate[k] <- shape/pred_mean[k]
